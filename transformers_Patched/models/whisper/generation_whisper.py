    ...
    def generate(
        ...
        **kwargs,
        ...
        while (seek < max_frames).any():
            ...
            time_offset = (
                seek.to(torch.float32 if device.type == "mps" else torch.float32) * time_precision / input_stride
            )
        ...
        if len(timestamp_segment_indices) > 0:
                ...
                segments.append(
                    {
                        ...
                        + start_timestamp_pos.to(torch.float32 if device.type == "mps" else torch.float32)
                        * time_precision,
                        "end": time_offset[prev_idx]
                        + end_timestamp_pos.to(torch.float32 if device.type == "mps" else torch.float32)
                        * time_precision,
            ...
            if timestamps.numel() > 0 and timestamps[-1] != timestamp_begin:
                ...
                last_timestamp_pos = (timestamps[-1] - timestamp_begin).to(
                    torch.float32 if device.type == "mps" else torch.float32
                )
            segments = [
                {
                    "start": time_offset[prev_idx],
